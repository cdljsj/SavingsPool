...
contract SavingsPool is Initializable {
    ...
    function deposit(uint depositAmount) mine external {
    ...
    function redeem(uint redeemAmount) mine external {
        require(fTokenBalances[msg.sender] > 0, "Balance shoule > 0");
    ...
    function claimMiningToken(address tokenHolder) mine public {
        uint miningTokenAmount = saverMiningAccrued[tokenHolder];
        if (miningTokenAmount == 0) return;
        saverMiningAccrued[tokenHolder] = 0;
        IERC20(miningToken).safeTransfer(msg.sender, miningTokenAmount);

        emit Claimed(msg.sender, miningToken, miningTokenAmount);
    }
